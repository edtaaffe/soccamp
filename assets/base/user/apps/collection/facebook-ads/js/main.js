...
jQuery(document).ready( function ($) {
    ...
    */ 
     ...
     * 
     * @param integer contains the page's number
     * 
     * @since   0.0.8.1
     */ 
    Main.search_for_interest_suggestions = function(page) {

        // Get search keys
        let key = $('.main .modal.show .search-for-targeting-suggestions').val();

        // Prepare the data to send
        var data = {
            action: 'search_for_targeting_suggestions',
            key: key,
            page: page
        };
        ...
        // Make ajax call
        Main.ajax_call(url + 'user/app-ajax/facebook-ads', 'POST', data, 'search_for_targeting_suggestions');
    ...
    ********************************/
     ...
     * @since   0.0.8.1
     */        
    $(document).on('keyup', '.main .modal.show .search-for-targeting-suggestions', function () {
        
        // Search for interest suggestions
        Main.search_for_interest_suggestions(1);
     ...
     * @since   0.0.8.1
     */ 
    $( document ).on( 'click', '.main .modal.show .btn-next, .main .modal.show .btn-previous', function (e) {
        e.preventDefault();
        ...
        // Search for interest suggestions
        Main.search_for_interest_suggestions(page);
    ...
    /*
     * Detect interest suggestion click
     * 
     * @param object e with global object
     * 
     * @since   0.0.8.1
     */ 
    $( document ).on( 'click', '.main .modal.show .list-targeting-suggestions .list-group-item', function (e) {
        e.preventDefault();
        
        // Get suggestion's id
        var suggestion_id = $(this).attr('data-id');
        
        // Get suggestion's text
        var suggestion_text = $(this).find('h5').text();

        // Verify if the suggestion already exists
        if ( $('.main .modal.show .interest-suggestions-selected .btn-primary[data-id="' + suggestion_id + '"]').length > 0 ) {

            // Remove suggestion
            $('.main .modal.show .interest-suggestions-selected .btn-primary[data-id="' + suggestion_id + '"]').remove();

        } else {

            // Verify if were selected more than 5 suggestions
            if ( $('.main .modal.show .interest-suggestions-selected .btn-primary').length > 4 ) {

                // Display alert
                Main.popup_fon('sube', words.only_five_interest_suggestions, 1500, 2000);                

            } else {

                // Add suggestion
                $('.main .modal.show .interest-suggestions-selected').append('<button type="button" class="btn btn-primary" data-id="' + suggestion_id + '">' + suggestion_text + '</button>');
    ...
    /*
     * Detect interest suggestion remove click
     * 
     * @param object e with global object
     * 
     * @since   0.0.8.1
     */ 
    $( document ).on( 'click', '.main .modal.show .interest-suggestions-selected .btn-primary', function (e) {
        e.preventDefault();
    ...
    Main.methods.create_ad_campaigns = function ( status, data ) {
        ...
        if ( data.response ) {
            ...
            if ( success ) {
                ...
                // Empty the interest suggestions list
                $('.main .modal.show .interest-suggestions-selected').empty();
                $('.main .modal.show .search-for-targeting-suggestions').val('');
        ...
        if ( status === 'success' ) {
            ...
            var overview = '<div class="row page-titles">'
                                ...
                                + '<div class="col-xl-6 text-right clean">'
                                    + '<button type="button" class="btn btn-success btncampaign" data-toggle="modal" data-target="#ads-create-new-ad">'
                                ...
                                + '<div class="table-responsive">'
                                    + '<table class="table">'
                                        + '<thead>'
                                            + '<tr>'
                                                + '<th scope="row" colspan="3">'
                                                    + '<button type="button" class="btn btn-success btncampaign" data-toggle="modal" data-target="#ads-create-ad-set">'
                                                        + '<i class="icon-puzzle"></i>'
    ...
    */
    ...
    /*
     * Display the found targeting suggestions
     * 
     * @param string status contains the response status
     * @param object data contains the response content
     * 
     * @since   0.0.8.1
     */
    Main.methods.search_for_targeting_suggestions = function ( status, data ) {

        if ( data.page < 2 ) {
                    
            $('.main .modal.show .btn-previous').addClass('btn-disabled');
            
        } else {
            
            $('.main .modal.show .btn-previous').removeClass('btn-disabled');
            $('.main .modal.show .btn-previous').attr('data-page', (parseInt(data.page) - 1));
            
        }
        
        if ( (parseInt(data.page) * 10 ) < data.total ) {
            
            $('.main .modal.show .btn-next').removeClass('btn-disabled');
            $('.main .modal.show .btn-next').attr('data-page', (parseInt(data.page) + 1));
            
        } else {
            
            $('.main .modal.show .btn-next').addClass('btn-disabled');
        ...
        if ( status === 'success' ) {
            ...
            for ( var d = 0; d < data.suggestions.length; d++ ) {
                ...
                all_suggestions += '<a href="#" class="list-group-item list-group-item-action" data-id="' + data.suggestions[d].id + '">'
                                        + '<div class="d-flex w-100 justify-content-between">'
                                            + '<h5 class="mb-1">'
                                                + data.suggestions[d].name
                                            + '</h5>'
                                            + '<small>'
                                                + data.suggestions[d].audience_size
                                            + '</small>'
                                        + '</div>'
                                    + '</a>';
            ...
            // Set no results
            all_suggestions += '<p class="no-results">'
                    + data.words.no_suggestions_found
                + '</p>';
             
        }

        // Display the response
        $('.main .modal.show .list-targeting-suggestions').html(all_suggestions);  
    ...
    ********************************/ 
    ...
    $(document).on('submit', '.main .facebook-ads-create-campaign', function (e) {
        ...
        // Define interest suggestions
        var interest_suggestions = [];

        // Verify if user has selected interest suggestions
        if ( $('.main .modal.show .interest-suggestions-selected .btn-primary').length > 0 ) {

            // Get interests
            let interests = $('.main .modal.show .interest-suggestions-selected .btn-primary');

            // List all interest suggestions
            for ( var i = 0; i < interests.length; i++ ) {
                
                interest_suggestions.push($(interests[i]).attr('data-id'));
                
            }

            // Set interest suggestions
            data['interest_suggestions'] = Object.entries(interest_suggestions);
